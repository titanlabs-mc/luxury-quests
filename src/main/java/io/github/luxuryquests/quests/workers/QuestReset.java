...
public class QuestReset {
    ...
    public void start() {
        ...
        Bukkit.getScheduler().runTaskLater(this.plugin, () -> {
            this.reset();
            this.start();
            Bukkit.getLogger().log(Level.SEVERE, "[LuxuryQuests] Scheduler has notified of reset for " + this.resetId);
    ...
    public void reset() {
        Bukkit.getLogger().log(Level.SEVERE, "[LuxuryQuests] Reset has occurred for " + this.resetId + ". Is proxymaster? " + this.proxyMaster);
        if (!this.proxyMaster) {
            Bukkit.getLogger().log(Level.SEVERE, "[LuxuryQuests] Start syncing async");
            this.plugin.wrappedScheduler().runDelay(ThreadContext.ASYNC, () -> {
                ...
                Bukkit.getLogger().log(Level.SEVERE, "[LuxuryQuests] Syncing complete");
            }, 400);
            return;
        }
        Bukkit.getLogger().log(Level.SEVERE, "[LuxuryQuests] Clearing all users " + this.questType + " quests and saving them");
